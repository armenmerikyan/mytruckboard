...
//			String url = "jdbc:sapdb://localhost/BIGDB?sqlmode=ORACLE";
		...
		Connection connection2 = DriverManager.getConnection(url,"<USERNAME>", "<PASSWORD>");
			...
			if(request.getParameter("isSentView")!=null && request.getParameter("isSentView").equals("YES")){
				ps = connection.prepareStatement("select * from mtb_approved_location_viewer WHERE approved_id = ? and client_id = ?");	
			}else{
				ps = connection.prepareStatement("select * from mtb_approved_location_viewer WHERE approved_id = ? and client_id_approved = ?");	
			}			
			ps.setString(1, request.getParameter("access_id"));
			ps.setString(2, clientid);
			ResultSet resultSet = ps.executeQuery();
			while (resultSet.next()) {
				emailAcess = NullCheck.check(resultSet.getString("email_approved"));
				nameAccess = NullCheck.check(resultSet.getString("name_approved"));
			}					
