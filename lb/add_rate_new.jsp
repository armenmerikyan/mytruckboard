...
================================================== -->
  ...
  <body onload="initialize()" >
		...
		<li><a href="truckloads_history.jsp?id=<%=request.getParameter("id")%>">Truckload History</a></li>
...
//							String url = "jdbc:sapdb://localhost/BIGDB?sqlmode=ORACLE";
 	 ...
 	 <p></P>
		...
		<H3><%=NullCheck.check(resultSet.getString("CONTACT_PERSON"))%> <small> ( <a href="truckloads_history.jsp?id=<%=request.getParameter("id")%>">Truckload History</a> )</small> </H3>
...
<%}%>	
    <%
       ...
       int carrier_pay = 0;
				...
				ps = connection.prepareStatement("select rate, miscel_pay, pickState, destState, pickCity, destCity, pickup_address,drop_address, pickup_notes,drop_notes,pickup_contact,drop_contact,pickup_phone,drop_phone,pickup_date,drop_date, X(pickcitylocation) as my_lat, Y(pickcitylocation) as my_long, X(destcitylocation) as dest_my_lat, Y(destcitylocation) as dest_my_long  from test.trucking_quote where quote_id = ? order by ts desc ");
    ...
    %>
      ...
      <tr>
        <th class="col-md-4">Pick From City</th>
        <th class="col-md-2">Date/Time</th>
		<th class="col-md-4">Deliver To City</th>
...
<div class="input-group">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1" id="pickCity" name="pickCity"   <%if(request.getParameter("quote_id")!=null){%>value="<%=pickCity%>" <%}else{%>value="<%=NullCheck.check((String)session.getAttribute("pickCity"))%>"<%}%> >
		...
		<span id="pickup_addressSPN" name="pickup_addressSPN" style="border-color:#262626;border-width: 1px;line-height:1.4em;font-size:1.0em;width:100%;">&nbsp;</span>
...
<div class="input-group input-group-md">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1" id="pickup_time" name="pickup_time" value="08:00" style="font-size: 14px; padding-left: 5px; padding-right: 2px; width: 90px;">
</div></div>  </td>
...
<div class="input-group">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1" id="destCity" name="destCity"  <%if(request.getParameter("quote_id")!=null){%>value="<%=destCity%>"<%}else{%>value="<%=NullCheck.check((String)session.getAttribute("destCity"))%>" <%}%> >
		...
		<div class="text-center">
		<span id="drop_addressSPN" name="drop_addressSPN" style="border-color:#262626;border-width: 1px;line-height:1.4em;font-size:1.0em;width:100%;">&nbsp;</span>
...
<div class="input-group">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1" id="drop_time" name="drop_time"  value="08:00" style="font-size: 14px; padding-left: 5px; padding-right: 2px; width: 90px;">
</div></div>  </td>
<td ><%=NullCheck.check((String)session.getAttribute("duration"))%></td>
		...
		<div class="col-md-5" style="padding-left: 8px; padding-right: 0px;">

			<h4 style="margin-top: 0px;">Pickup Notes:</h4>
			<textarea rows="3" class="form-control" type="text" id="pickup_notes" name="pickup_notes"  value="<%=pickNotes%>"></textarea>
			</div>
		...
		<h4 style="margin-top: 0px;">Dropoff Notes:</h4>
		<textarea rows="3" class="form-control" type="text" id="drop_notes" name="drop_notes" value="<%=destNotes%>"></textarea>
		</div>
		...
		<h4 style="margin-top: 15px;">Contact Name:</h4>
		<input type="text" class="form-control"  aria-describedby="basic-addon1" id="pickup_contact" name="pickup_contact"  value="<%=pickContact%>">

		</div>

		<div class="col-md-2" style="padding-left: 8px; padding-right: 0px;">

		<h4 style="margin-top: 15px;">Phone:</h4>
		<input type="text" class="form-control"  aria-describedby="basic-addon1" id="pickup_contact_phone" name="pickup_contact_phone" value="<%=pickPhone%>">
		...
		<h4 style="margin-top: 15px;">Contact Name:</h4>
		<input type="text" class="form-control"  aria-describedby="basic-addon1" id="drop_contact" name="drop_contact"  value="<%=destContact%>">
		...
		<h4 style="margin-top: 15px;">Phone:</h4>
		<input type="text" class="form-control"  aria-describedby="basic-addon1" id="drop_contact_phone" name="drop_contact_phone" value="<%=destPhone%>">
...
<div class="col-md-12" style="padding-top: 20px;"></div>
		...
		<div class="panel" id="line_item_detail" name="line_item_detail" style="padding-top: 20px; margin-bottom: 0px;">
...
<div class="input-group"><a class="btn btn-primary" href="javascript:void(0)" role="button" onclick="addLineItem('addItem');"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></a>
      ...
      <%
				ps = connection.prepareStatement("select * from test.trucking_quote_lineitem where quote_id = ? order by ts desc ");
				ps.setString(1, request.getParameter("quote_id"));
				 resultSet = ps.executeQuery();
...
<div class="input-group"><a class="btn btn-primary" href="javascript:void(0)" role="button" onclick="addLineItem('removeItem','<%=resultSet.getString("line_item_id")%>');"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></a>
      ...
      <%}%>
		...
		<div class="panel-heading"><div class="col-md-10"></div>
		<div class="input-group col-md-2"><a class="btn btn-primary" href="javascript:void(0)" role="button" onclick="autoSaveItem();"><span class="glyphicon glyphicon-save" aria-hidden="true"></span> SEND TRUCKLOAD</a>
		</div>
		</div>
		</div>
	  </div>  
