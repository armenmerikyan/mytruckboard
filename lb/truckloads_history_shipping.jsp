...
================================================== -->
  <body onload="initialize()" >
    ...
    <li class="active"><a href="add_rate.jsp?id=<%=request.getParameter("id")%>">Truckloads</a></li>
...
<br/><br/>
  ...
  <table class="table table-hover">
    ...
    <%
							...
							PreparedStatement psQ = connection.prepareStatement("select reject_status, completed_status, accept_status, pickup_date, drop_date, trucking_quote_shipping.quote_id, image_load_id, trucking_quote_shipping.ts, user_id, miscel_pay, miles, rate, truck_mpg, driver_pay_mpg, avg_diesel, status, pickCity, pickState, destCity, destState, (GLength(LineStringFromWKB(LineString(pickCityLocation, GeomFromText('POINT(" + request.getParameter("pickCity_lat") + " " + request.getParameter("pickCity_lng") + ")'))))) AS distanceFromPickCity, (GLength(LineStringFromWKB(LineString(pickCityLocation, GeomFromText('POINT(" + request.getParameter("destCity_lat") + " " + request.getParameter("destCity_lng") + ")'))))) AS distanceFromDestCity, (GLength(LineStringFromWKB(LineString(destCityLocation, GeomFromText('POINT(" + request.getParameter("pickCity_lat") + " " + request.getParameter("pickCity_lng") + ")'))))) AS ddistanceFromPickCity, (GLength(LineStringFromWKB(LineString(destCityLocation, GeomFromText('POINT(" + request.getParameter("destCity_lat") + " " + request.getParameter("destCity_lng") + ")'))))) AS ddistanceFromDestCity from test.trucking_quote_shipping LEFT JOIN maids_image_load ON trucking_quote_shipping.quote_id = maids_image_load.quote_id where equipment_id = ? order by ts desc  LIMIT 2400");
...
%>
 <tr>
	<td><a href="add_rate_shipping.jsp?id=<%=request.getParameter("id")%>&quote_id=<%=resultSetQ.getString("quote_id")%>" ><%if(resultSetQ.getString("pickCity")==null || resultSetQ.getString("pickCity").equals("")){%>No City<%}else{%><%=resultSetQ.getString("pickCity")%><%}%></a></td>
