...
%>
	...
	document.getElementById(selectBox).style.backgroundColor = "orange";
		...
		xmlhttp.open("GET", "add_shipper_new_load_detail.changeStatus.agent.jsp" + responseString,true);
		xmlhttp.send();	
    }
	...
	<script type="text/javascript">
      ...
      function clearFieldsNowCity() {
        ...
        function viewInvoicePayments(load_id) {
					...
					responseString = responseString + "&new_fin_type=" + encodeURIComponent("Agent Payment");		
...
<div class="modal fade" id="myModalInv" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1"  name="new_invoice_payment_amount" id="new_invoice_payment_amount" value="" placeholder="">
				...
				for(int j=0;j<rowCount;j++){
					...
					if(request.getParameter("IPTAMTCK" + j)!=null){
						...
						if(fin_amount!=0.0f){
							...
							ps.setString(6, "Auto Batch Manager");
							ps.setString(7, "Agent Payment");
	...
	<div class="container-fluid">
		...
		<%
	         try{
				...
				String url = "jdbc:mysql://localhost/test?zeroDateTimeBehavior=convertToNull";
				Connection connection = DriverManager.getConnection(url,"<USERNAME>", "<PASSWORD>");						
				PreparedStatement psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and (load_status_agent= 'Open' or load_status_agent is null)");
	...
	<form class="form-inline" name="searchFormBatch" id="searchFormBatch" action="dispatch_board_accounting_agent.jsp?ShowSearchAccountingContractor=true" method="post">
		...
		<%
			...
			while (resultSet.next()) {
				...
				if(!resultSetMap.containsKey(resultSet.getString("load_board_id"))){
					...
					rowArray[13] = resultSet.getString("load_status_agent");	
			...
			<% float pen_pay_rate = 1.0f;
				...
				try{
					float gross_rate = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[24]));
					float gross_rate_agent = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[27]));
					float gross_rate_sales = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[23]));
					commission = gross_rate_agent * Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[7]));
					pen_pay_rate = gross_rate + gross_rate_agent + gross_rate_sales;
					pen_pay_rate = gross_rate_agent/pen_pay_rate;
