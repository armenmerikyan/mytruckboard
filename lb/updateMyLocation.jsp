...
//			System.out.println(request.getParameter("clientid"));
		...
		Connection connection = DriverManager.getConnection(urldb);
			...
			ps = connection.prepareStatement("update maids_customers set location = GeomFromText(?), address1 = ?, city = ?, state = ?, zip = ?, speedCurrent = ?, ts_last_update = ?, directionCurrent = ?,  wtg = ?, basicNotes = ?, specialNotes = ?, price_per_mile = ?, current_state = ? where client_id = ? ");
			...
			ps.setString(9, request.getParameter("wtge"));
			ps.setString(10, request.getParameter("bdetail"));
			ps.setString(11, request.getParameter("sdetail"));
			ps.setFloat(12, Float.parseFloat(price_per_mile));
			ps.setString(13, request.getParameter("orderTYpe"));
