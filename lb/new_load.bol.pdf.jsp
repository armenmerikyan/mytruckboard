...
//            document.add(new Paragraph("Load Number: LN"+ ln_number));        
	   ...
	   if(totalParties<=2){
	   	...
	   	totalPageCount = ((totalParties - 2)/2) + 1 ;
 	...
 	Font boldFont = new Font(Font.FontFamily.TIMES_ROMAN, 18, Font.BOLD);
        PdfPCell cellTitle = new PdfPCell(new Phrase("BILL OF LADING\n\n", boldFont));
 	...
 	celladd.setRowspan(8);
        ...
        float[] columnWidthCarrier = {3,2,2,2};
 	...
 	celladdSHPnote.setColspan(3);
        ...
        currentPageCount += 1; 
 	Font boldFontNameLegal = new Font(Font.FontFamily.TIMES_ROMAN, 8, Font.BOLD);
 	Font boldFontNameLegalReg = new Font(Font.FontFamily.TIMES_ROMAN, 8, Font.NORMAL);
        document.add(new Phrase("NOTE (1) Where the rate depends on value, shippers must state specifically in writing the agreed or declared value of the property as follows: The agreed or declared value of the property is hereby specifically stated by the shipper to be not exceeding\n\n", boldFontNameLegal));          
	document.add(new Phrase("Note: (2) Liability limitation for loss or damage on this shipment may be applicable. See 49 U.S.C. �14706 (c)(1)(A) and (B).\n", boldFontNameLegal));
	document.add(new Phrase("Received subject to individually determined rates or written contracts that have been agreed on in writing between the carrier and shipper, if applicable, otherwise to rates, classifications and rules that have been established by the carrier and are available to the shipper on request.\n\n" , boldFontNameLegalReg));
	document.add(new Phrase("Note (3) Products requiring special or additional care or attention in handling or stowing must be so marked and packaged as to ensure safe transportation with ordinary care. See Sec. 2(e) of NMFC item 360.",  boldFontNameLegal)); 
	document.add(new Phrase(" If this shipment is to be delivered to the consignee without recourse on the consignor, the consignor shall sign the following statement: The carrier may decline to make delivery of this shipment without payment of freight and all other lawful charges. Signature of consignor", boldFontNameLegalReg)); 
        document.close();
