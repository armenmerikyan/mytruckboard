...
%>
    ...
    <!-- <script>
       ...
       <!-- Google api:-->
		...
		Connection connection = DriverManager.getConnection(urldb);
			...
			PreparedStatement psMain = connection.prepareStatement("select producttype, key_public, phone, f_name, city, zip, state, email, X(location) as city_lat, Y(location) as city_lng from maids_oltp.maids_customers where producttype is null");
            		if(request.getParameter("searchKey")!=null && !request.getParameter("searchKey").equals("")){
				psMain = connection.prepareStatement("select producttype, key_public, phone, f_name, city, zip, state, email, X(location) as city_lat, Y(location) as city_lng from maids_oltp.maids_customers where maids_customers.f_name like ? and producttype is null");
 ...
 /*
    ...
    */
        ...
        //alert(urlString);
        xmlhttp.open("GET", "trucklist-detail_current.jsp?origin=" + encodeURIComponent(startLoc) + "&lat=" + lat +"&long=" + long + urlString,true);
        xmlhttp.send();
        document.getElementById("defaultdd").innerHTML = "<nav><ul class=\"pager\"><img src=\"images/loading.gif\" style=\"width:5.1em;vertical-align:middle;\" /></ul></nav>";
        }
        ...
        xmlhttp.send();
        document.getElementById("defaultDetail").innerHTML = "<nav><ul class=\"pager\"><img src=\"images/loading.gif\" style=\"width:5.1em;vertical-align:middle;\" /></ul></nav>";
        }
        </script>
...
<!--
    ...
    <li class="list-group-item list-group-item-sm" style="padding:5px;">
  <button style="width:100%;" onclick="getDistanceTrucks('<%=dataSetFirst[0]%>','<%=dataSetFirst[4]%>','<%=dataSetFirst[5]%>')" type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <%if(NullCheck.check(dataSetFirst[6]).equals("")|| NullCheck.check(dataSetFirst[6]).equals("null")){%>[No City Name]<%}else{%><%=dataSetFirst[6].toUpperCase()%>, <%=dataSetFirst[7]%><%}%>
	
	<span class="badge pull-right" style="background-color:black;color:white;"><%=mainTruckLayer.size() %>
	</span>         
  </button>
</li>
    ...
    <div class="col-sm-12" >
    <div class="col-md-12" style=" padding: 0px;border-width: 0px;border-style: solid;border-color:white;background-color:white;">
    <div class="col-md-7" style="padding-right: 5px; padding-left: 0px;">
    <div class="panel panel-default">
    <div class="panel-body" >
    <div style="float:left;height:460px;width:100%;" id="map-canvas"></div>
    </div>
    </div>
    </div>
    <div id="defaultDetail" class="col-sm-5" style="max-height: 460px; padding-left: 2px; padding-right: 2px; ">

	<div class="text-center" style="color:black;">
    <h1><span class="fa fa-truck" aria-hidden="true" style="color:gray;"></span> <%=j +84%></h1>
    <h2>REGISTERED<BR> TRUCK DRIVERS</h2>
    ...
    <p>myTruckBoard Driver App <BR> <a href="https://itunes.apple.com/us/app/mytruckboard-driver/id1044398785?mt=8" >more info</a>
            ...
            <div class="col-md-12" >
            <form class="form-inline" name="searchForm" id="searchForm" action="truckboard_current.jsp" method="post">
            <label class="sr-only" for="exampleInputAmount">SelectForm</label>
            <div class="input-group input-group-md" style="padding:5px;">
            <div class="input-group-addon" style="color:green;" ><span class="glyphicon glyphicon-sort-by-attributes" aria-hidden="true"></span>&nbsp;ORIGIN&nbsp;&nbsp;</div>
     ...
     <input type="text" class="form-control" id="pac-input"  name="pac-input" placeholder="City Name" value="<%=NullCheck.check((String)session.getAttribute("pac-input"))%>" />
            </div>
            <div class="input-group input-group-md" style="padding:5px;">
	    <div class="input-group-addon" style="color:orange;" ><span class="glyphicon glyphicon-filter" aria-hidden="true"></span>&nbsp;DRIVER&nbsp;&nbsp;</div>
            <input type="text" oninput="autoFilterKey()" id="searchKey" name="searchKey" style="font-size: 14px; padding-left: 5px; width: 110px;" class="form-control" placeholder="First Name" <%if(request.getParameter("searchKey")!=null){%> value="<%=request.getParameter("searchKey")%>"<%}%>>
            <!--
            &nbsp;<button class="btn btn-default btn-md" type="button" onclick="this.form.submit()" >FILTER</button>
            &nbsp;<button class="btn btn-default btn-md" type="button" onclick="resetFormSearch()" >CLEAR</button>
            -->
            </div>
            <div class="input-group input-group-md" style="padding:5px;">           
	           <div class="input-group-addon" style="color:red;" ><span class="glyphicon glyphicon-flag" aria-hidden="true"></span>&nbsp;DESTINATION&nbsp;&nbsp;</div>
	            <input type="text" class="form-control" id="pac-input2" name="pac-input2" placeholder="City Name" value="<%=NullCheck.check((String)session.getAttribute("pac-input2"))%>" />
            </div>
	</div>
<!--
...
<hr style="margin-top: 5px; margin-bottom: 5px;">
    ...
    <h4>
<h2>Supplement your fleet!</h2>
<h4 style="color:green;">To list drivers near a city START by entering an ORIGIN city</h4>
    </h4>
    </div>
    </div>
    </div>
