...
if(uuid !=null){
		 try{
			...
			Connection connection = DriverManager.getConnection(url,"<USERNAME>", "<PASSWORD>");	
				...
				ps = connection.prepareStatement("select * from test.trucking_quote_shipping where quote_id = ? order by ts desc ");
				...
				if(isNewQuote){
					ps = connection.prepareStatement("INSERT INTO test.trucking_quote_shipping(quote_id, ts, user_id, company_id, rate, miles, truck_mpg, avg_diesel, driver_pay_mpg, miscel_pay,equipment_id,status, pickCity, pickState, pickCityLocation, destCity, destState, destCityLocation, pickup_date, drop_date, load_type, pickup_address, pickup_notes, drop_address, drop_notes, pickup_contact, drop_contact, pickup_phone, drop_phone ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,GeomFromText(?),?,?,GeomFromText(?),?,?,?,?,?,?,?,?,?,?,?)");
					ps.setString(1, request.getParameter("quote_id"));			
					...
					ps = connection.prepareStatement("Update test.trucking_quote_shipping Set rate = ?, miles = ?, truck_mpg = ?, avg_diesel = ?, driver_pay_mpg = ?, miscel_pay = ?, pickCity = ?, pickState = ?, pickCityLocation = GeomFromText(?), destCity = ?, destState = ?, destCityLocation = GeomFromText(?), pickup_date = ?, drop_date = ?, load_type = ?, pickup_address = ?, pickup_notes = ?, drop_address = ?, drop_notes = ?, pickup_contact = ?, drop_contact = ? , pickup_phone = ?, drop_phone = ?      where quote_id = ?");
					ps.setInt(1, rate);	
					ps.setInt(2, miles);
