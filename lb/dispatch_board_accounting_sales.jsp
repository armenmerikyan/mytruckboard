...
%>
	...
	document.getElementById(selectBox).style.backgroundColor = "orange";
		...
		xmlhttp.open("GET", "add_shipper_new_load_detail.changeStatus.sales.jsp" + responseString,true);
		xmlhttp.send();	
    }
    ...
    function updateLoadCounts() {
		...
		{
			var addressList = xmlhttp.responseText.split("<ITEM>");
			document.getElementById("divCountDelivered").innerHTML = addressList[5];
			document.getElementById("divCountCompleted").innerHTML = addressList[6];
			document.getElementById("divCountInvoiced").innerHTML = addressList[7];
			document.getElementById("divCountPaid").innerHTML = addressList[8];
		}
		}
		xmlhttp.open("GET", "dispatch_board_counts.payments.jsp" ,true);
		xmlhttp.send();	
    }       
    function initialize() {
	    rotater();
	...
	<script type="text/javascript">
      ...
      function clearFieldsNowCity() {
        ...
        function viewInvoicePayments(load_id) {
					...
					responseString = responseString + "&new_fin_type=" + encodeURIComponent("Sales Payment");		
        ...
        function addNewInvoicePaymentQuick(load_id) {
					...
					xmlhttp.onreadystatechange=function()
					  ...
					  if (xmlhttp.readyState==4 && xmlhttp.status==200 )
						...
						document.getElementById(invPaymentBoxNameTotal).value = amtDue.toFixed(2);	
		...
		var rowCount = parseInt(document.getElementById("rowCount").value);
	        ...
	        for(i=0;i<rowCount;i++) {
	        	...
	        	if(document.getElementById(checkBoxNameCK).checked == true){
	        		...
	        		var checkBoxName = "IPTAMTID" + i;
	        		var checkBoxValue  = "IPTAMT" + document.getElementById(checkBoxName).value
	        		var tmpTotal = parseFloat(document.getElementById(checkBoxValue).value);
	        		total = total + tmpTotal;
	        	}
	        }	          	        
	        document.getElementById("batch_total").value = total.toFixed(2);
...
<div class="modal fade" id="myModalInv" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <input type="text" class="form-control"  aria-describedby="basic-addon1"  name="new_invoice_payment_amount" id="new_invoice_payment_amount" value="" placeholder="">
	         ...
	         try{
				...
				for(int j=0;j<rowCount;j++){
					...
					if(request.getParameter("IPTAMTCK" + j)!=null){
						...
						if(!request.getParameter("IPTAMT" + request.getParameter("IPTAMTID" + j)).equals(""))fin_amount = Float.parseFloat(request.getParameter("IPTAMT" + request.getParameter("IPTAMTID" + j)).trim());
						if(fin_amount!=0.0f){
							ps = connection.prepareStatement("INSERT INTO test.load_board_finance(fin_id, ts, company_id, created_by, load_id, fin_name, fin_type, fin_date, fin_amount, fin_name_note, fin_name_notesms) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
							ps.setString(1, UUID.randomUUID() + "");
							ps.setTimestamp(2, ts);
							ps.setString(3, uuid);
							ps.setString(4, (String)session.getAttribute("login_id"));
							ps.setString(5, request.getParameter("IPTAMTID" + j));
							ps.setString(6, "Auto Batch Sales");
							ps.setString(7, "Sales Payment");
	...
	<div class="container-fluid">
		...
		<%
	         try{
				...
				String url = "jdbc:mysql://localhost/test?zeroDateTimeBehavior=convertToNull";
				Connection connection = DriverManager.getConnection(url,"<USERNAME>", "<PASSWORD>");						
				PreparedStatement psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and (load_status_sales= 'Open' or load_status_sales is null)");
	...
	<a href="dispatch_board_accounting.jsp?searchType=LoadStatus&searchKey=Delivered&ShowSearchAccounting=true">
					...
					<a href="dispatch_board_accounting.jsp?searchType=LoadStatus&searchKey=Complete&ShowSearchAccounting=true">
					<div class="col-lg-3 col-md-3 col-sm-3 col-sx-6">
					<div class="panel panel-primary">
					...
					<a href="dispatch_board_accounting.jsp?searchType=LoadStatus&searchKey=Invoiced&ShowSearchAccounting=true">
					<div class="col-lg-3 col-md-3 col-sm-3 col-sx-6">
					<div class="panel panel-primary">
					...
					<a href="dispatch_board_accounting.jsp?searchType=LoadStatus&searchKey=Archived&ShowSearchAccounting=true">
					<div class="col-lg-3 col-md-3 col-sm-3 col-sx-6">
					<div class="panel panel-primary">
	...
	<form class="form-inline" name="searchFormBatch" id="searchFormBatch" action="dispatch_board_accounting_sales.jsp?ShowSearchAccountingContractor=true" method="post">
	...
	<tbody style="font-size:13px;">
		...
		<%
			...
			HashMap resultSetMapPaymentsBonus = new HashMap();
			HashMap resultSetMapPaymentsPen = new HashMap();
			...
			while (resultSet.next()) {
				...
				if(request.getParameter("searchType")!=null){
					...
					if(request.getParameter("searchType").equals("customer")){
							psMain = connection.prepareStatement("select TRUCKING_EQUIPMENT.name, load_board.load_board_id, SUM(load_board_finance.fin_amount) as all_total_paid from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id left join test.load_board_finance on load_board.load_board_id = load_board_finance.load_id where load_board.company_id = ? and load_board_finance.fin_type = 'Sales Bonus Payment' and TRUCKING_EQUIPMENT.name like ?  group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, "%" + request.getParameter("searchKey") + "%");						
					}
					if(request.getParameter("searchType").equals("mc_number")){
							psMain = connection.prepareStatement("select TRUCKING_EQUIPMENT.mc_no, TRUCKING_EQUIPMENT.name, load_board.load_board_id, SUM(load_board_finance.fin_amount) as all_total_paid from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id left join test.load_board_finance on load_board.load_board_id = load_board_finance.load_id where load_board.company_id = ? and load_board_finance.fin_type = 'Sales Bonus Payment' and TRUCKING_EQUIPMENT.mc_no like ?  group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));					
					}
				}	
				resultSet = psMain.executeQuery();
				while (resultSet.next()) {
					resultSetMapPaymentsBonus.put(resultSet.getString("load_board_id"), String.format("%.02f", resultSet.getFloat("all_total_paid")));
				}				
				...
				if(request.getParameter("searchType")!=null){
					...
					if(request.getParameter("searchType").equals("customer")){
							psMain = connection.prepareStatement("select TRUCKING_EQUIPMENT.name, load_board.load_board_id, SUM(load_board_finance.fin_amount) as all_total_paid from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id left join test.load_board_finance on load_board.load_board_id = load_board_finance.load_id where load_board.company_id = ? and load_board_finance.fin_type = 'Penalty Charge' and TRUCKING_EQUIPMENT.name like ?  group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, "%" + request.getParameter("searchKey") + "%");						
					}
					if(request.getParameter("searchType").equals("mc_number")){
							psMain = connection.prepareStatement("select TRUCKING_EQUIPMENT.mc_no, TRUCKING_EQUIPMENT.name, load_board.load_board_id, SUM(load_board_finance.fin_amount) as all_total_paid from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id left join test.load_board_finance on load_board.load_board_id = load_board_finance.load_id where load_board.company_id = ? and load_board_finance.fin_type = 'Penalty Charge' and TRUCKING_EQUIPMENT.mc_no like ?  group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));					
					}
				}	
				resultSet = psMain.executeQuery();
				while (resultSet.next()) {
					resultSetMapPaymentsPen.put(resultSet.getString("load_board_id"), String.format("%.02f", resultSet.getFloat("all_total_paid")));
				}	
			...
			while (resultSet.next()) {
				...
				if(!resultSetMap.containsKey(resultSet.getString("load_board_id"))){
					if((resultSet.getString("salDetail")!=null && resultSet.getString("salDetail").contains((String)session.getAttribute("login_id")))
					){
						resultSetVector.add(resultSet.getString("load_board_id"));
					}
					String[] rowArray = new String[28];
					...
					rowArray[13] = resultSet.getString("load_status_sales");	
					...
					if(resultSetMapPaymentsBonus.containsKey(resultSet.getString("load_board_id"))){
						rowArray[25] = (String)resultSetMapPaymentsBonus.get(resultSet.getString("load_board_id"));	
					}else{
						rowArray[25] = "0.00";	
					}
					if(resultSetMapPaymentsPen.containsKey(resultSet.getString("load_board_id"))){
						rowArray[26] = (String)resultSetMapPaymentsPen.get(resultSet.getString("load_board_id"));	
					}else{
						rowArray[26] = "0.00";	
					}
					rowArray[23] = resultSet.getString("gross_rate_sales");
					rowArray[24] = resultSet.getString("gross_rate_manager");
					rowArray[27] = resultSet.getString("gross_rate_agent");
			...
			<% float pen_pay_rate = 1.0f;
					
				try{
					float gross_rate = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[23]));
					float gross_rate_agent = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[24]));
					float gross_rate_sales = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[27]));
					commission = gross_rate * Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[7]));
					pen_pay_rate = gross_rate + gross_rate_agent + gross_rate_sales;
					pen_pay_rate = gross_rate/pen_pay_rate;
					pen_pay_rate = pen_pay_rate * Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[26]));
	...
	<%}%>				
			...
			<td style="padding:5px;"><%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[7])%></td>
			<td style="padding:5px;">
			<input type="hidden" name="IPTAMTBON<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" id="IPTAMTBON<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" value="<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[20])%>" />
			<input type="hidden" name="IPTAMTPEN<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" id="IPTAMTPEN<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" value="<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[21])%>" />
			<%=String.format("%.02f", commission)%>
			<% 
			
			float amount_due = commission - Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[19]));
			amount_due = amount_due + Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[25]));
			amount_due = amount_due - pen_pay_rate;
			float bonus_amount = Float.parseFloat(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[25]));
			%>
			</td>
			<td style="padding:5px;">
			<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[25])%></td>
			<td style="padding:5px;">
			<%=String.format("%.02f", pen_pay_rate)%></td>
			<input type="hidden" name="IPTAMTCOM<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" id="IPTAMTCOM<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" value="<%=String.format("%.02f", commission + bonus_amount - pen_pay_rate)%>" />
			...
			<td style="padding:5px;width:100px;"><div class="form-group" ><input type="text" class="form-control"  name="IPTAMT<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" id="IPTAMT<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" value="<%=String.format("%.02f", amount_due)%>" onchange="calculateRate()" placeholder="1200.00" readonly/></div></td>
	...
	<select name="select<%=((String[])resultSetMap.get((String)resultSetVector.get(l)))[0]%>" id="select<%=((String[])resultSetMap.get((String)resultSetVector.get(l)))[0]%>" onchange="updateLoadStatus('<%=((String[])resultSetMap.get((String)resultSetVector.get(l)))[0]%>',this.value)" >
        ...
        <option <%if(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[13]).equals("Processing")){%>selected<%}%> value="Processing" name="load_status_label" id="load_status_label">Processing</option>
        <option <%if(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[13]).equals("Closed")){%>selected<%}%> value="Closed" name="load_status_label" id="load_status_label">Closed</option>
        <option <%if(NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[13]).equals("Archived")){%>selected<%}%> value="Archived" name="load_status_label" id="load_status_label">Archived</option>
        </select>		
			</td>
			</tr>				
