...
//			System.out.println(request.getParameter("clientid"));
		...
		Connection connection = DriverManager.getConnection(urldb);
			...
			while (resultSetSec.next()) {
				...
				ps = connection.prepareStatement("INSERT INTO mtb_mylocations(mylocation_id, client_id, TS, mylocation_name, mylocation_email, mylocation_location, mylocation_address, mylocation_city, mylocation_state, mylocation_zip, ispublicviewable, isfriendsviewable, mylocation_id_public, mylocation_type ) VALUES (?,?,?,?,?,GeomFromText(?),?,?,?,?,?,?,?,?)");
				ps.setString(1, UUID.randomUUID() + "");
				ps.setString(2, clientid);
				ps.setTimestamp(3, ts);
				ps.setString(4, request.getParameter("name"));
				ps.setString(5, request.getParameter("email"));
				ps.setString(6, latLong);
				if(!responseText.equals(""))ps.setString(7, responseText.trim().split(",")[0]);
				if(responseText.equals(""))ps.setString(7, "");
				if(!responseText.equals(""))ps.setString(8, responseText.trim().split(",")[1].trim());
				if(responseText.equals(""))ps.setString(8, "");
				if(!responseText.equals(""))ps.setString(9, responseText.trim().split(",")[2].split(" ")[1].trim());
				if(responseText.equals(""))ps.setString(9, "");
				if(!responseText.equals(""))ps.setString(10, responseText.trim().split(",")[2].split(" ")[2].trim());
				if(responseText.equals(""))ps.setString(10, "");
				ps.setBoolean(11, isPublicView);
				ps.setBoolean(12,isFriendsView);
				ps.setString(13, UUID.randomUUID() + "");
				ps.setString(14, request.getParameter("placeType"));
				System.out.println("Update Location Status : " + ps.executeUpdate());				
			
			ps = connection.prepareStatement("update maids_customers set location = GeomFromText(?), address1 = ?, city = ?, state = ?, zip = ? where client_id = ? ");

		connection.close();		     
		     //System.out.println("lat/lng=" + lat + "," + lng);
		}catch(Exception ex){
			%>Unable To Translate To Address  <ERRORSAVING> <%=ex + ""%><%
			System.out.println(ex + "");
		}
		%><SAVEDLOCATION><%if(!responseText.equals("")){%><%=responseText.trim().split(",")[1].trim() + ", " + responseText.trim().split(",")[2].split(" ")[1].trim()%><%}else{%>Update Error<%}%>		