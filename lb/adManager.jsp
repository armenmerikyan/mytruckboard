...
//			String url = "jdbc:sapdb://localhost/BIGDB?sqlmode=ORACLE";
	...
	try{
			...
			PreparedStatement ps = connection.prepareStatement("INSERT INTO test.ads(ad_id, company_id, ts, LOCATION, city, state, address1, address2, zip, name, ad_name, daily_max, total_max, ad_message,start_date, radius, city_all) VALUES (?,?,?,GeomFromText(?),?,?,?,?,?,?,?,?,?,?,?,?,?)");
			ps.setString(1, adUUID);
			ps.setString(2, uuid);
			...
			ps.setString(4, latLong);
			ps.setString(5, cityv);
			ps.setString(6, statev);
			ps.setString(7, request.getParameter("address1"));
			ps.setString(8, request.getParameter("address2"));
			ps.setString(9, request.getParameter("zip"));
			ps.setString(10, request.getParameter("adName"));
			ps.setString(11, request.getParameter("ad_name"));
			ps.setFloat(12, dailyLimit);
			ps.setFloat(13, totalLimit);
			ps.setString(14, request.getParameter("displayText"));
			ps.setDate(15, date_mod);
			ps.setInt(16,radiusLimit);
			ps.setString(17, cityall);
...
<!-- Main page: -->
        ...
        <h3>myTruckBoard iOS and Android App ads network <a href="adManager.list.jsp" >list</a></h3>
        <hr>
        <div class="row">
        ...
        <form action="adManager.jsp" id="addtruckform" name="addtruckform" method="post" class="form-horizontal" role="form" data-toggle="validator">
        ...
        <div class="col-sm-8">
        <input class="form-control" type="text" name="radiusLimit"  placeholder="Radius" required>
        </div>
        </div>
        ...
        <div class="col-sm-8">
        <input class="form-control" type="text" name="adName"  placeholder="Name" required>
        </div>
        </div>
        ...
        <div class="col-sm-8">
        <input class="form-control" type="text" name="displayText" placeholder="Phone Number, email or web address">
        </div>
        </div>
        <div class="form-group">
        ...
        <div class="col-sm-8">		<%
					...
					%>
						...
						<INPUT type="checkbox" name="<%=resultSet.getString("result_id")%>" > <%=resultSet.getString("name")%> (<%=resultSet.getString("count")%>)<BR/>
					<%
						rowCount += 1;
					}
			connection.close();
...
<INPUT type="checkbox" name="ad_nd" value="Yes" > New Drivers on the Platform<BR/> 
<INPUT type="checkbox" name="ad_hd" value="Yes" > Hungry Drivers <BR/>
<INPUT type="checkbox" name="ad_td" value="Yes" > Tired Drivers <BR/>
<INPUT type="checkbox" name="ad_id" value="Yes" > Insurance About to Expire <BR/>
<INPUT type="checkbox" name="ad_sd" value="Yes" > Safe Drivers <BR/>
<INPUT type="checkbox" name="ad_ed" value="Yes" > Empty Drivers <BR/>
<INPUT type="checkbox" name="ad_pd" value="Yes" > Partial Load <BR/>
<INPUT type="checkbox" name="ad_od" value="Yes" > Old Vehicles <BR/>
<INPUT type="checkbox" name="ad_nd" value="Yes" > Needs Gas         
-->
        </div>
        </div>
         <div class="form-group">
        ...
        <div class="col-sm-8">
        <input class="form-control" type="text" name="dailyLimit"  placeholder="$50.00" required>
        </div>
        </div>    
         <div class="form-group">
        <label class="col-sm-3 control-label">Set a total budget (optional):</label>
        <div class="col-sm-8">
        <input class="form-control" type="text" name="totalLimit"  placeholder="$200.00" >
        </div>
        </div> 
        ...
        <input type="submit" class="btn btn-primary btn-lg" value="Create Advertisement" name="submit1" onclick="addtrucksubmit();">
        <span></span>
