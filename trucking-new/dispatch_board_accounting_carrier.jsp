	...
	<script type="text/javascript">
      ...
      function clearFieldsNowCity() {
        ...
        function viewInvoicePayments(load_id) {
					...
					responseString = responseString + "&new_fin_type=" + encodeURIComponent("Carrier Payment");		
	...
	<div class="container-fluid">
		...
		<%
	         try{
				...
				PreparedStatement psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and (load_status_carrier= 'Open' or load_status_carrier is null)");
				psMain.setString(1, uuid);
				ResultSet resultSet = psMain.executeQuery();
				while (resultSet.next()) {
					totalCountDelivered = resultSet.getInt("itemCount");
				}
				psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and load_status_carrier= 'Processing'");
				psMain.setString(1, uuid);
				resultSet = psMain.executeQuery();
				while (resultSet.next()) {
					totalCountCompleted = resultSet.getInt("itemCount");
				}
				psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and load_status_carrier= 'Closed'");
				psMain.setString(1, uuid);
				resultSet = psMain.executeQuery();
				while (resultSet.next()) {
					totalCountInvoiced = resultSet.getInt("itemCount");
				}
				psMain = connection.prepareStatement("select count(load_board_id) as itemCount from test.load_board WHERE load_board.COMPANY_ID = ? and load_status_carrier= 'Archived'");
				psMain.setString(1, uuid);
				resultSet = psMain.executeQuery();
				while (resultSet.next()) {
	...
	<tbody style="font-size:13px;">
		...
		<%
			...
			while (resultSet.next()) {
				...
				psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id where load_board.company_id = ? order by load_board.ts asc");
				psMain.setString(1, uuid);
				if(request.getParameter("searchType")!=null){
					if(request.getParameter("searchType").equals("wo_number")){						
							psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id where load_board.company_id = ? and load_board.load_wo = ? group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));						
					}
					if(request.getParameter("searchType").equals("load_number")){						
							psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id where load_board.company_id = ? and load_board.load_number = ? group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));						
					}
					if(request.getParameter("searchType").equals("LoadStatus")){						
							psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id where load_board.company_id = ? and load_board.load_status = ? group by  load_board.load_board_id order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));						
					}
					if(request.getParameter("searchType").equals("customer")){		
							psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id WHERE load_board.COMPANY_ID = ? and TRUCKING_EQUIPMENT.name like ? order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, "%" + request.getParameter("searchKey") + "%");					
					}
					if(request.getParameter("searchType").equals("mc_number")){		
							psMain = connection.prepareStatement("select * from test.load_board left join test.TRUCKING_EQUIPMENT on load_board.load_equipment_id=TRUCKING_EQUIPMENT.id WHERE load_board.COMPANY_ID = ? and TRUCKING_EQUIPMENT.mc_no like ? order by load_board.ts asc");			
							psMain.setString(1, uuid);
							psMain.setString(2, request.getParameter("searchKey"));						
					}		
				}
			...
			while (resultSet.next()) {
				...
				if(!resultSetMap.containsKey(resultSet.getString("load_board_id"))){
					...
					float tmpTotal = (resultSet.getFloat("carrier_rate") + resultSet.getFloat("load_carrier_fee")) - (resultSet.getFloat("advances_total") + tmpPaid);	
			...
			<td style="padding:5px;"><%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[16])%></td>
			...
			<td style="padding:5px;width:100px;"><div class="form-group" ><input type="text" class="form-control"  name="IPTAMT<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" id="IPTAMT<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[0])%>" value="<%=NullCheck.check(((String[])resultSetMap.get((String)resultSetVector.get(l)))[17])%>" onchange="calculateRate()" placeholder="1200.00" readonly/></div></td>
